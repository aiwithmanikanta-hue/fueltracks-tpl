        ...
        <div className="mt-14 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {items.map((it, i) => (
            <Reveal key={it.title} delay={(i % 4) * 0.06}>
              <div className="group relative aspect-[4/5] rounded-2xl overflow-hidden bg-white/40 border border-white/20 cursor-pointer hover-lift">
                <img
                  src={it.img}
                  alt={it.alt}
                  loading="lazy"
                ...
                <div className="absolute inset-0 ring-1 ring-inset ring-white/10 rounded-2xl group-hover:ring-primary/50 transition-colors" />
                <div className="absolute inset-x-0 bottom-0 p-5">
                  <h3 className="font-semibold text-lg text-white drop-shadow">{it.title}</h3>
                  <p className="text-xs mt-0.5 text-slate-200/90">{it.desc}</p>
